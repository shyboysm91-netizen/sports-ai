...
function koreanVenue(value: string) {
  const map: Record<string, string> = {
    ...
    "Kurashiki": "구라시키 머스캣 스타디움",
    "Matsuyama": "마쓰야마 봇짱 스타디움",
    "Naha": "오키나와 셀룰러 스타디움 나하",
  };
...
function parseGameText(
  ...
  const between = text
    .slice(first.index + first.alias.length, second.index)
    .replace(/\s+/g, " ")
    .trim();

  // NPB 공식 결과 페이지:
  // 첫 번째 팀 = 홈팀, 두 번째 팀 = 원정팀
  // 예: Hanshin 0 Game 11 Koshien 7 DeNA
  const scoreMatch = between.match(
    /^(\d{1,2})\s+Game\s+\d+\s+(.+?)\s+(\d{1,2})$/i,
  );
  ...
  const venue = koreanVenue(scoreMatch[2]);
  const awayScore = Number(scoreMatch[3]);

  const requestedSide: Game["requestedSide"] =
    requestedTeam === homeTeam ? "home" : "away";

  const runsFor = requestedSide === "home" ? homeScore : awayScore;
  const runsAgainst = requestedSide === "home" ? awayScore : homeScore;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const game of collected) {
      unique.set(`${game.date}-${game.homeTeam}-${game.awayTeam}-${game.homeScore}-${game.awayScore}`, game);
    }
    ...
    return NextResponse.json({
      success: true,
      source: "NPB 공식 경기 결과",
      parserVersion: "recent-games-v2-home-away",
      team: team.ko,
