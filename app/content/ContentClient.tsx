...
export default function ContentPage() {
  ...
  async function uploadYoutube() {
    ...
    try {
      const response = await fetch("/api/content/youtube/upload-session", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ title:`${data.away} vs ${data.home} 경기 분석`, description:caption || data.summary, privacyStatus:youtubePrivacy, mimeType:youtubeFile.type || "video/webm", size:youtubeFile.size }) });
      const json = await response.json();
...
function buildNarration(d: ContentData, headline: string) {
  ...
  return [
    ...
    `AI 분석 결과 ${predicted} 우세, 예상 점수 ${d.awayScore} 대 ${d.homeScore}입니다.`,
    `더 자세한 분석은 장군분석.kr에서 확인하세요.`,
  ];
}
...
function drawCard(ctx: CanvasRenderingContext2D, index: number, d: ContentData, headline: string) {
  ...
  if (index === 5) {
    ...
    ctx.fillStyle = "#8ea0b8"; ctx.font = "800 28px Arial";
    wrap(ctx, "매일 업데이트되는 야구 분석은 장군분석.kr에서 확인하세요.", 70, 1040, 920, 42);
  }
...
function drawFooter(ctx: CanvasRenderingContext2D, index: number) {
  ...
  ctx.fillStyle = "#ffffff"; ctx.font = "900 24px Arial"; ctx.fillText("장군분석.kr", 70, 1300);
...
function ctaBox(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
  ...
  ctx.font = "900 38px Arial"; ctx.fillText("장군분석.kr", x + 45, y + 125);
