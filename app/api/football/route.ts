...
const LEAGUES = {
  ...
  ucl: { name: "UEFA 챔피언스리그", espn: "uefa.champions" },
...
async function espnGames(league: LeagueKey, date: string) {
  ...
  return events.map((event: any) => {
    ...
    const hasScore = statusType === "STATUS_FINAL" || statusType === "STATUS_IN_PROGRESS" || statusType === "STATUS_HALFTIME";
    return {
      ...
      homeScore: hasScore ? home?.score ?? null : null, awayScore: hasScore ? away?.score ?? null : null,
...
async function kLeagueGames(date: string) {
  ...
  return (data?.scheduleList ?? []).filter((game: any) => game.gameDate === dotted).map((game: any) => ({
    ...
    homeId: String(game.homeTeam || ""), awayId: String(game.awayTeam || ""),
