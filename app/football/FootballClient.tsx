...
export default function FootballClient() {
  ...
  return <main className="min-h-screen bg-[#020817] text-white">
    <header className="border-b border-slate-800 bg-[#050b19]"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5"><Link href="/football" className="text-2xl font-black">장군 AI 축구</Link><div className="flex gap-2"><Link href="/news" className="rounded-full border border-cyan-800 px-4 py-2 text-sm text-cyan-300">스포츠 뉴스</Link><Link href="/" className="rounded-full border border-slate-700 px-4 py-2 text-sm text-slate-300">야구 홈</Link></div></div></header>
    <section className="mx-auto max-w-6xl px-5 py-10">
      ...
      <div className="mt-6 grid gap-5 md:grid-cols-2">{games.map(game=><article key={`${game.league}-${game.id}`} className="overflow-hidden rounded-2xl border border-slate-700 bg-[#0f172a]">
        ...
        <Link href={`/football/analysis/${game.league}/${game.date}/${encodeURIComponent(game.id)}/${encodeURIComponent(`${game.home}-대-${game.away}`)}?leagueName=${encodeURIComponent(game.leagueName)}&homeId=${encodeURIComponent(game.homeId)}&awayId=${encodeURIComponent(game.awayId)}&time=${encodeURIComponent(game.time)}&venue=${encodeURIComponent(game.venue)}`} className="m-5 mt-0 block rounded-xl bg-emerald-500 py-3 text-center font-black text-slate-950">경기 분석 보기</Link>
      </article>)}</div>
    </section>
  </main>;
}
